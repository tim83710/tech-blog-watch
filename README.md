...
每日 digest 開頭另附兩段快訊列點，用 Gemini 的 Google Search grounding 即時搜尋、總結過去 48 小時的動態（昨日已報導過的會自動排除，避免重複）：

- 「**AI 產業脈動**」：大廠 blog 以外的產業大事（新勢力模型、爆紅開源專案、主權 AI、晶片、融資、監管）
- 「**金融×AI 脈動**」：AI 在金融業的落地與影響（金融機構×AI 公司合作、銀行/資管導入 AI、AI 投資/交易/風控應用、金融監管 AI）

列點呈現、來源連結附在後面；大廠 blog 沒新文章的日子也會單獨發這兩段。

另外**每週三**（台北時間）加一段「**本週 GitHub 專案**」：從 Hacker News 高分討論與 GitHub Trending（weekly）蒐集候選，由 Gemini 依口味（prompt 見 `prompts/github-weekly.md`）挑最多 5 個值得認識的專案寫繁中介紹。介紹過的 repo 記在 `state.json`，之後只有在「隔了冷卻期、且發了新的 GitHub Release」時才會以「（更新）」形式再次出現，並說明這次更新了什麼。
...
GitHub Actions (每天 06:30 台北排程；實際常因 GitHub 排程延遲而晚到)
  ...
  → pulse.py    脈動段：Gemini + Google Search grounding 產「AI 產業脈動」與「金融×AI 脈動」
                （規則各在 prompts/industry-pulse.md、prompts/finance-ai-pulse.md）
  → github_watch.py（僅週三）HN 高分 + GitHub Trending → Gemini 選題寫「本週 GitHub 專案」
...
| `prompts/industry-pulse.md` | AI 產業脈動 prompt（**單一事實來源**，改該段風格/範圍改這裡） |
| `prompts/finance-ai-pulse.md` | 金融×AI 脈動 prompt（**單一事實來源**，改該段風格/範圍改這裡） |
...
| `main.py` | 串起流程 |
| `state.json` | 已看過的文章 URL + 昨日脈動列點 + 介紹過的 GitHub repo（每天由 Actions commit 更新） |
...
- `--force-github`：不管星期幾都跑 GitHub 週段（測試用，通常搭配 `--dry-run`）
...
- **改脈動風格/範圍**：AI 產業段改 `prompts/industry-pulse.md`、金融×AI 段改 `prompts/finance-ai-pulse.md`；關掉某段：`sources.yaml` 的 `pulse_enabled` / `finance_pulse_enabled` 設 `false`
- **改 GitHub 週段**：選題口味改 `prompts/github-weekly.md`；星期幾出、幾個專案、HN 分數門檻、重覆冷卻天數都在 `sources.yaml` 的 `github_weekly_*` / `github_hn_min_points` / `github_refeature_days`；關掉設 `github_weekly_enabled: false`
- **加/減來源**：`sources.yaml`